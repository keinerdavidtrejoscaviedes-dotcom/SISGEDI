...
class DocumentoController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $rolLower = mb_strtolower(trim($usuario['rol'] ?? ''));

        // Retornar vista específica o redireccionar basado en el id_rol
        // 1. Aprendiz -> Panel del Aprendiz
        if (($usuario['id_rol'] ?? null) == 27 || $rolLower === 'aprendiz') {
            return redirect()->route('sisgedi.aprendiz.panel');
        }

        // 2. Gerente Comercial -> Dashboard Comercial
        if (($usuario['id_rol'] ?? null) == 4 || str_contains($rolLower, 'comercial')) {
        ...
        foreach ($ultimasConvocatorias as $c) {
            $actividadReciente->push([
                ...
                'tiempo' => $c->fecha_apertura
                    ? 'Abierta desde ' . Carbon::parse($c->fecha_apertura)->format('d M Y')
                    : 'Sin fecha',
            ]);
        }
