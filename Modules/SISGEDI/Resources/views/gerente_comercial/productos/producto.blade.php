...
<div x-data="portafolio()" class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        ...
        <button type="button" @click="openModal('create')" class="bg-[#39A900] hover:bg-[#2e8a00] text-white px-5 py-2.5 rounded-lg font-medium transition-colors flex items-center gap-2 shadow-md shadow-[#39A900]/20">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4">
        ...
        <select x-model="filters.category" class="px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#39A900] outline-none bg-white text-sm transition-all w-full md:w-56">
            ...
            @endforeach
            <!-- Fallback in case there are no categories from DB yet -->
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="flex justify-between items-start mb-4">
                        <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold tracking-wide"
                              :class="{
                                  ...
                                  'bg-slate-100 text-slate-700 border border-slate-300': !['Producción del centro', 'Emprendedores', 'Asociaciones campesinas', 'Egresados', 'Talentos SENA'].includes(producto.categoria)
        ...
        <!-- Empty State -->
        <div x-show="filteredProductos.length === 0" x-cloak class="col-span-full py-16 bg-white rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
    ...
    <div x-show="isModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
            ...
            <!-- Modal Panel -->
            <div class="relative inline-block bg-white rounded-2xl text-left shadow-2xl sm:my-8 sm:max-w-2xl w-full border border-slate-100 overflow-visible">
                <form @submit.prevent="saveProducto">
                        ...
                        <div class="space-y-5">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div class="col-span-full">
                                    ...
                                    <p x-show="errors.nombre" class="text-xs text-red-500 mt-1" x-text="errors.nombre?.[0]"></p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p x-show="errors.categoria" class="text-xs text-red-500 mt-1" x-text="errors.categoria?.[0]"></p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p x-show="errors.precio" class="text-xs text-red-500 mt-1" x-text="errors.precio?.[0]"></p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p x-show="errors.origen" class="text-xs text-red-500 mt-1" x-text="errors.origen?.[0]"></p>
                                </div>
                                
                                <!-- Aliado -->
                                <div x-show="form.origen !== 'Producción propia' && form.origen !== ''">
                                    ...
                                    <p x-show="errors.aliado" class="text-xs text-red-500 mt-1" x-text="errors.aliado?.[0]"></p>
                                </div>
                    ...
                    <div class="bg-slate-50 px-6 py-4 flex flex-col-reverse sm:flex-row-reverse gap-3 rounded-b-2xl border-t border-slate-100">
                        <button type="submit" class="w-full sm:w-auto inline-flex justify-center items-center rounded-lg px-6 py-2.5 bg-[#39A900] text-sm font-bold text-white hover:bg-[#2e8a00] transition-colors shadow-md shadow-[#39A900]/20 disabled:opacity-50" :disabled="loading">
                            <i class="fas fa-save mr-2" :class="{'fa-spinner fa-spin': loading}"></i> <span x-text="loading ? 'Guardando...' : 'Guardar Producto'"></span>
                        </button>
                        <button type="button" @click="isModalOpen = false" class="w-full sm:w-auto inline-flex justify-center items-center rounded-lg px-6 py-2.5 bg-white text-sm font-bold text-slate-700 border border-slate-300 hover:bg-slate-50 transition-colors shadow-sm" :disabled="loading">
                            Cancelar
                        </button>
                    </div>
                </form>
    ...
    <div x-show="isDeleteModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen p-4 text-center sm:p-0">
            <!-- Background Overlay -->
            <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" @click="isDeleteModalOpen = false"></div>
            
            <!-- Modal Panel -->
            <div class="relative inline-block bg-white rounded-2xl text-left shadow-2xl sm:my-8 sm:max-w-lg w-full border border-slate-100 overflow-visible">
                ...
                <div class="bg-slate-50 px-6 py-4 flex flex-col-reverse sm:flex-row-reverse gap-3 rounded-b-2xl border-t border-slate-100">
                    <button type="button" @click="executeDelete()" class="w-full sm:w-auto inline-flex justify-center items-center rounded-lg px-6 py-2.5 bg-rose-600 text-sm font-bold text-white hover:bg-rose-700 transition-colors shadow-md shadow-rose-600/20 disabled:opacity-50" :disabled="loading">
                        <i class="fas fa-power-off mr-2" :class="{'fa-spinner fa-spin': loading}"></i> Sí, Desactivar
                    </button>
                    <button type="button" @click="isDeleteModalOpen = false" class="w-full sm:w-auto inline-flex justify-center items-center rounded-lg px-6 py-2.5 bg-white text-sm font-bold text-slate-700 border border-slate-300 hover:bg-slate-50 transition-colors shadow-sm" :disabled="loading">
                        Cancelar
                    </button>
                </div>
            </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('portafolio', () => ({
            ...
            async saveProducto() {
                ...
                let url = '{{ route("sisgedi.comercial.producto.store") }}';
                let method = 'POST';
                
                if (this.modalMode === 'edit') {
                    // Sustituir param ID dinámicamente o usar un formato base y concatenar
                    url = `{{ url('/sisgedi/comercial/producto') }}/${this.form.id}`;
                    method = 'PUT';
                }
                ...
                try {
                    ...
                    if (response.status === 422) {
                        ...
                        if (this.modalMode === 'create') {
                            this.productos.unshift(data.producto);
                        } else {
                            const index = this.productos.findIndex(p => p.id === this.form.id);
                            if (index !== -1) {
                                this.productos[index] = data.producto;
                            }
                        }
            ...
            async executeDelete() {
                ...
                this.loading = true;
                const url = `{{ url('/sisgedi/comercial/producto') }}/${this.productoToDelete.id}`;
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (index !== -1) {
                            // En vez de borrarlo, actualizamos su estado a Inactivo como se pidió
